...
Class Action {
	...
	function login(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM `admin` where username = '".$username."' ");
		if($qry->num_rows > 0){
	...
	function login2(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM user where email = '".$email."' ");
		if($qry->num_rows > 0){
			$result = $qry->fetch_array();
			$is_verified = password_verify($password, $result['password']);
			if($is_verified){
	...
	function signup(){
		...
		$data .= ", email = '$email' ";
		$data .= ", password = '$password' ";
		$chk = $this->db->query("SELECT * FROM user where email = '$email' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
