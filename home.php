 <!-- Masthead-->
            ...
            <div class="container h-100">
                <div class="row h-100 align-items-center justify-content-center text-center">
                    <div class="col-lg-10 align-self-center mb-4 page-title">
                    	<h1 class="text-white">Welcome to Awang Ansiah Nursery</h1>
                        <hr class="divider my-4 bg-dark" />
                        <a class="btn btn-dark bg-black btn-xl js-scroll-trigger" href="#product">Order Now</a>
	...
	<section class="page-section" id="product">
        <h1 class="text-center text-cursive" style="font-size:3em"><b>Product</b></h1>
        <div class="d-flex justify-content-center">
            <hr class="border-dark" width="5%">
        </div>
        <div id="product-field" class="card-deck mt-2">
                <?php 
                    include 'admin/db_connect.php';
                    $limit = 10;
                    $page = (isset($_GET['_page']) && $_GET['_page'] > 0) ? $_GET['_page'] - 1 : 0 ;
                    $offset = $page > 0 ? $page * $limit : 0;
                    $all_product =$conn->query("SELECT id FROM  product_list")->num_rows;
                    $page_btn_count = ceil($all_product / $limit);
                    $qry = $conn->query("SELECT * FROM  product_list order by `name` asc Limit $limit OFFSET $offset ");
                    while($row = $qry->fetch_assoc()):
                    ?>
                    <div class="col-lg-3 mb-3">
                     <div class="card product-item  rounded-0">
    ...
    <?php if(isset($_GET['_page'])): ?>
        <script>
            $(function(){
                document.querySelector('html').scrollTop = $('#product').offset().top - 100
            })
        </script>
    <?php endif; ?>
